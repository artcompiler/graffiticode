...
var Ast = (function () {
  ...
  Ast.setPool = function setPool(pool) {
    nodePool = pool;
    numberMap = {};
    stringMap = {};
    nodeMap = {};
  ...
  Ast.prototype.node = function node(nid) {
    trace("Ast.node() nid=" + nid);
    //trace("pool:" + Ast.dumpAll());
    var s = JSON.stringify(nodePool[nid]);
    trace("s=" + s);
    var n = JSON.parse(s);
    //var node = this.create(n);
    // if literal, then unwrap.
    switch (n.op.toLowerCase()) {
    case "num":
    case "str":
      n = n.args[0];
      break;
...
var Model = (function () {
  ...
  var render = function render(n) {
    trace("Ast.render() n=" + JSON.stringify(n, null, 2));
    var text = "";
    if (typeof n === "string") {
      text = n;
    } else if (typeof n === "number") {
...
//  console.log(str)
  ...
  function render(pool) {
    trace("render() " + JSON.stringify(pool, null, 2));
    Ast.setPool(pool);
    var node = new Ast().node(pool.length-1);
    var model = new Model();
    var str = ""
    str += prefix();
    str += model.toLaTeX(node);
    str += suffix()
    trace("render() str=" + str);
    return str
  }
